...
click_heli_dshort = (((543, 529), (40, 40), 40), 0.2, 0.5, False)
...
def runMap(firstOrder):
    ...
    runSequence([PL_map1, startMap, PL_mapStarted, click_heli_dshort, PL_mapStarted, click_heli_dshort, PL_resupply, resupply, PL_mapStarted, planning, PL_planning, click_command, PL_commandSelected, randomMapDrag(), PL_dragged])
...
def micro(index):
    ...
    #don't wait as long for the snipers to trigger
    if index == 2:
        wait(1.2, 0.2)
    ...
    #do corner kite and back
    if index == 2:
        wait(1.666, 0.2)
        if checkPixels(PL_battle):
            reposition(posGrid[5], posGrid[1])
            wait(1.2, 0.2)
            reposition(posGrid[1], posGrid[5])
    else:
        wait(1.4, 0.2)
        if checkPixels(PL_battle):
